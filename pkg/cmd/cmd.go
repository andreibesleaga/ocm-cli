	...
	Command = &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&requestflag.Flag[string]{
				Name:    "api-key",
				Sources: cli.EnvVars("OCM_API_KEY"),
			},
			&requestflag.Flag[string]{
				Name:    "api-key-header",
				Sources: cli.EnvVars("OCM_API_KEY"),
			...
			&requestflag.Flag[string]{
				Name:    "bearer",
				Sources: cli.EnvVars("OCM_USERNAME"),
			},
		},
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "ocm @manpages [-o ocm.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "ocm.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "ocm.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
