...
import (
	...
	"github.com/stainless-sdks/ocm-cli/internal/jsonview"
...
func getDefaultRequestOptions(cmd *cli.Command) []option.RequestOption {
	opts := []option.RequestOption{
		option.WithHeader("User-Agent", fmt.Sprintf("Ocm/CLI %s", Version)),
	...
	if cmd.IsSet("api-key-header") {
		opts = append(opts, option.WithAPIKeyHeader(cmd.String("api-key-header")))
	}
	if cmd.IsSet("bearer") {
		opts = append(opts, option.WithBearer(cmd.String("bearer")))
	}
